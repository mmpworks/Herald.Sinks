...
Sink kind: `trace` (the identifier the Dashboard form and JSON config use to reference this sink).

## Capabilities

- Zero external dependencies — BCL only
- Integrates with any TraceListener in Trace.Listeners
- Optional category prefix for listener filtering
...
- Output visibility depends on a registered TraceListener
...
- **AOT-compatible**: yes
- **Targets**: .NET 8 / 9 / 10
- **Thread safety**: Thread-safe — Trace.WriteLine is internally synchronized.

## Vendor

Microsoft — https://learn.microsoft.com/dotnet/api/system.diagnostics.trace
