# MMP.Herald.Sinks.Exceptionless
...
```bash
dotnet add package MMP.Herald.Sinks.Exceptionless
```

The sink auto-registers into `LogSinkProviderRegistry.Default` via a `[ModuleInitializer]` on assembly load. No manual `RegisterAll(...)` or `With*SinkProviders()` call is required — `dotnet add package` is the whole workflow.

Sink kind: `exceptionless` (the identifier the Dashboard form and JSON config use to reference this sink).

## Capabilities

- Bulk POST per IBatchedLogSink
- Bearer-token auth
- Event type "log" vs "error" auto-detected from context exception
...
Exceptionless — https://exceptionless.com/docs/
